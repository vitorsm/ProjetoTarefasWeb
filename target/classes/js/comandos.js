...
var $simuladosAbertos = [];
var $simuladosFechados = [];

angular.module("TarefasApp", []).controller("Controller", function() {
	var self = this;
	self.nome = "Tarefa teste";
	self.foto = "Tarefa foto";
	self.localizacao = "Tarefa localiza";
});
...
function abrirListarSimulado ($id) {
	var $fechado = true;
	var $aberto = false;
	
	for ($i = 0 ; $i < $simuladosAbertos.length ; $i++) {
		if ($idDiv == $simuladosAbertos[$i]){
			$fechado = false;
		}
	}
	for ($i = 0 ; $i < $simuladosFechados.length ; $i++) {
		if ($idDiv == $simuladosFechados[$i]){
			$aberto = true;
		}
	}
	if ($fechado) {
		abrirTarefa($id);
		$simuladosAbertos[$simuladosAbertos.length] = $idDiv;
		document.getElementById($idDiv).className = 'listaSimuladosExibido';
		for ($i = 0 ; $i < $simuladosFechados.length ; $i++) {
			if ($idDiv == $simuladosFechados[$i]){
				$simuladosFechados[$i] = "";
			}
...
function fecharListarSimulado ($idDiv){
	$simuladosFechados[$simuladosFechados.length] = $idDiv;
	document.getElementById($idDiv).className = 'listaSimulados';
}
function formataCPF(Ncampo){
